...
export const knowledge: Record<TopicId, TopicKnowledge> = {
  ...
  mllec2: {
    cheat: [
      {
        formula: 'Ax = 0, Ay = 0 ⟹ A(x + y) = 0 and A(λx) = 0',
        why: 'Why the lecture moves on. Answers to a homogeneous system survive adding and scaling; answers to Ax = b, b ≠ 0, do not — two of them add to a solution of Ax = 2b.',
      },
      {
        formula: 'group: closure · associativity · neutral e · inverse',
        why: 'The four promises of (G, ⊗). Add x ⊗ y = y ⊗ x and it is an Abelian group. All four or none — three out of four buys nothing.',
      },
      {
        formula: '(ℤ, +) Abelian · (ℕ₀, +) no inverses · (ℤ, ·) no inverses',
        why: 'The deck’s three tests. (ℤ, ·) does have the identity 1; only 1 and −1 have inverses in it.',
      },
      {
        formula: '𝒱 = (𝒱, +, ·), + : 𝒱 × 𝒱 → 𝒱, · : ℝ × 𝒱 → 𝒱',
        why: 'A vector space is one inner operation and one outer one. The outer one takes its inputs from two different sets, which is why it is listed separately.',
      },
      {
        formula: 'λ·(x + y) = λx + λy   and   (λ + ψ)·x = λx + ψx',
        why: 'Distributivity, both ways round. The second is the one a wrong scaling rule usually breaks first.',
      },
      {
        formula: 'λ·(ψ·x) = (λψ)·x   and   1·x = x',
        why: 'Associativity and the neutral element of the outer operation. 1·x = x is easy to forget and easy to break — λ·x = 2λx breaks it while keeping both distributive laws.',
      },
      {
        formula: '0 = [0, 0, …, 0]ᵀ',
        why: 'The neutral element of (𝒱, +). Every vector space and every subspace has to contain it.',
      ...
      {
        formula: 'ℝⁿ · ℝᵐˣⁿ · polynomials · a layer’s weight matrix',
        why: 'All vector spaces. “Vector” is a job description — anything you can add and scale entry by entry qualifies.',
      },
      {
        formula: '𝒰 ⊆ 𝒱 is a subspace ⟺ 𝒰 ≠ Φ, λx ∈ 𝒰, x + y ∈ 𝒰',
        why: 'The short test. 𝒰 inherits associativity, distributivity, commutativity and the neutral element from 𝒱, so only closure can fail.',
      },
      {
        formula: '𝒰 ≠ Φ and λx ∈ 𝒰 ⟹ 0 ∈ 𝒰',
        why: 'Take λ = 0. This is why any set missing the origin fails at once — the line w₁ = 1, and the set of unit-norm weights.',
      },
      {
        formula: '0 ∈ 𝒰 is necessary, not sufficient',
        why: 'The square −1 ≤ w ≤ 1 contains the origin and still fails: stretch far enough and you leave. PCA needs centring for exactly this reason.',
      },
      {
        formula: 'null(A) = {x : Ax = 0} is always a subspace',
        why: 'The one the deck singles out. In a model it is the set of weight changes no data can detect, which is what makes coefficients non-unique.',
      },
      {
        formula: 'v = λ₁x₁ + λ₂x₂ + ⋯ + λₖxₖ',
        why: 'A linear combination. 0 is always reachable with every λ = 0 — the trivial combination, which proves nothing about any set of vectors.',
      },
      {
        formula: '0 = Σᵢ λᵢxᵢ with some λᵢ ≠ 0 ⟹ dependent',
        why: 'Linear dependence. Independent means the trivial combination is the only one that reaches 0.',
      },
      {
        formula: 'any set containing the zero vector is dependent',
        why: 'Give 0 a non-zero coefficient and everything else zero. A constant-zero feature column makes a design matrix rank-deficient on its own.',
      },
      {
        formula: 'all non-zero: dependent ⟺ one is a combination of the others',
        why: 'Slide 17. The “all non-zero” condition is what the previous line rules out, so quote both together.',
      },
      {
        formula: 'vectors as columns → row-reduce → pivot columns',
        why: 'The practical test. Pivot columns are an independent set; every non-pivot column is a combination of the pivot columns to its left.',
      },
      {
        formula: 'row-echelon form is not unique; pivot positions are',
        why: 'Scale any row and you get a different valid echelon form. Rank and the verdict never change. The reduced form, with leading 1s and zeros above, is unique.',
      },
      {
        formula: 'xⱼ = Bλⱼ ⟹ (x’s independent ⟺ λ’s independent)',
        why: 'Slides 22–25, and it needs the columns of B to be independent. Test the recipe vectors instead of the vectors themselves.',
      },
      {
        formula: 'Σⱼ ψⱼxⱼ = B(Σⱼ ψⱼλⱼ) = Bv, and Bv = 0 ⟹ v = 0',
        why: 'The proof in one line. v = 0 only follows because the columns of B are independent — that is where the hypothesis is used.',
      },
      {
        formula: '7c₁ + 15c₂ + 18c₃ + c₄ = 0',
        why: 'The deck’s four-vector answer. The coefficients are the last column of the reduced form, (−7, −15, −18, 0)ᵀ, with the signs flipped.',
      },
      {
        formula: 'm > k ⟹ dependent, always',
        why: 'At most min(k, m) pivots, so a column is left without one. Works from the shape alone. The converse is not a theorem — m ≤ k decides nothing.',
      },
      {
        formula: 'span[𝒜] = all linear combinations of 𝒜',
        why: 'Always defined and always a subspace. 𝒜 is a generating set of 𝒱 only when span[𝒜] happens to be all of 𝒱.',
      },
      {
        formula: 'basis = linearly independent generating set',
        why: 'Slide 34. Equivalently: a minimal generating set, or a maximal independent set. Independent alone is not enough.',
      },
      {
        formula: 'x = Σ λᵢbᵢ = Σ ψᵢbᵢ ⟹ λᵢ = ψᵢ',
        why: 'Coordinates in a basis are unique. That is what makes a basis an address system rather than just a spanning set.',
      },
      {
        formula: 'a basis is never unique; its size always is',
        why: 'ℝ³ has the canonical basis, (1,0,0),(1,1,0),(1,1,1), and endlessly many others. All have three members, and that number is dim(ℝ³).',
      },
      {
        formula: 'dim(𝒰) = number of vectors in a basis of 𝒰',
        why: 'Not the number of components. span{(0,1)ᵀ} has vectors with two components and dimension 1.',
      },
      {
        formula: '𝒰 ⊆ 𝒱 ⟹ dim(𝒰) ≤ dim(𝒱), equality ⟺ 𝒰 = 𝒱',
        why: 'Slide 38. The equality case is the standard trick for proving two spaces are the same without comparing members.',
      },
      {
        formula: 'basis of span(x₁…xₘ): columns → REF → keep pivot columns',
        why: 'Slide 40, in three steps. Keep the ORIGINAL vectors at the pivot positions, not the columns of the echelon form.',
      },
      {
        formula: 'slide 41: rank 3, pivots 1, 2, 4, x₃ = −x₁ + 2x₂',
        why: 'The example the deck sets and never answers. The four vectors are NOT a basis; {x₁, x₂, x₄} is one, and dim 𝒰 = 3. Checked by substituting back.',
      },
    ],
    quiz: [
      {
        q: 'The solutions of a linear system Ax = b form a vector space for which values of b?',
        options: [
          'Every b, since solutions can always be added',
          'Only b = 0',
          'Only when A is square',
          'Only when A has full rank',
        ],
        answer: 1,
        explain:
          'If Ax = b and Ay = b then A(x + y) = 2b, which equals b only when b = 0. So the solution set is closed under addition exactly for the homogeneous system. For b ≠ 0 the solution set is a shifted subspace — the reason ridge regression, not the data, has to break the tie when a design matrix is rank-deficient.',
      },
      {
        q: 'Which property does (ℕ₀, +) fail, making it not a group?',
        options: ['Closure', 'Associativity', 'The identity element', 'Inverses'],
        answer: 3,
        explain:
          'Slide 4 names this one. Sums of counting numbers are counting numbers, addition is associative, and e = 0 is in the set. But undoing +5 needs −5, which is not in ℕ₀. Three promises out of four is not a group — the definition is all or nothing.',
      },
      {
        q: 'A scaling rule is defined by λ ⊙ x = 2λx. Which vector space law does it break?',
        options: [
          'Distributivity over vector addition',
          'Distributivity over scalar addition',
          'Associativity of the outer operation, and 1 · x = x',
          'None — it is a valid vector space',
        ],
        answer: 2,
        explain:
          'Both distributive laws survive: 2λ(x + y) = 2λx + 2λy, and 2(λ + ψ)x = 2λx + 2ψx. But λ ⊙ (ψ ⊙ x) = 4λψx while (λψ) ⊙ x = 2λψx, and 1 ⊙ x = 2x is not x. This is why all four laws are listed — they are genuinely independent of one another.',
      },
      {
        q: 'The square −1 ≤ x ≤ 1, −1 ≤ y ≤ 1 in ℝ² contains the origin. Is it a subspace?',
        options: [
          'Yes, containing the origin is the test',
          'No — it is not closed under scaling',
          'No — it is not closed under addition',
          'Yes, because it is symmetric about the origin',
        ],
        answer: 1,
        explain:
          'Slide 12’s example. Containing 0 is necessary but nowhere near sufficient: take any non-zero vector in the square and scale it by 100 and you are outside. This is the exam trap — quote closure under scaling, not the presence of the origin.',
      },
      {
        q: 'Why is the nullspace {x : Ax = 0} a vector subspace for every matrix A?',
        options: [
          'Because A is invertible',
          'Because A(x + y) = Ax + Ay = 0 and A(λx) = λ(Ax) = 0, and A0 = 0',
          'Because it is the solution set of a system',
          'Because the rows of A are independent',
        ],
        answer: 1,
        explain:
          'Three lines of linearity, and none of them looks at A at all — which is why it holds for every matrix of every shape. In a model this is the set of weight changes that leave every prediction unchanged, so a non-trivial nullspace means the coefficients cannot be interpreted individually.',
      },
      {
        q: 'A set of vectors includes the zero vector. What can you say about it?',
        options: [
          'It is independent if the others are',
          'It is linearly dependent, whatever the others are',
          'It depends on how many vectors there are',
          'Nothing without doing elimination',
        ],
        answer: 1,
        explain:
          'Slide 17. Give the zero vector coefficient 1 and everything else 0: that is a non-trivial combination reaching 0. Practically, a feature column that is constant at zero makes a design matrix rank-deficient by itself, before any other collinearity.',
      },
      {
        q: 'Elimination on three vectors gives pivots in columns 1 and 3. What follows?',
        options: [
          'All three vectors are independent',
          'Vectors 1 and 3 are independent, and vector 2 is a combination of vector 1',
          'Vectors 1 and 3 are dependent',
          'Nothing until the reduced form is computed',
        ],
        answer: 1,
        explain:
          'Pivot columns are an independent set, and every non-pivot column is a combination of the pivot columns to its LEFT — here that is only column 1. This is exactly slide 19, where column 2 turns out to be twice column 1.',
      },
      {
        q: 'Two people row-reduce the same matrix and get different row-echelon forms. Who is wrong?',
        options: [
          'Whoever did not scale the pivots to 1',
          'Neither — echelon form is not unique, but the pivot positions are',
          'Both, since the echelon form is unique',
          'Whoever swapped rows',
        ],
        answer: 1,
        explain:
          'Scaling a row by any non-zero number gives another valid echelon form. What cannot differ is where the pivots sit, so rank and the independence verdict are the same either way. The REDUCED row-echelon form, with leading 1s and zeros above as well as below, is unique.',
      },
      {
        q: 'The x vectors are built as xⱼ = Bλⱼ. When is "x’s independent ⟺ λ’s independent" valid?',
        options: [
          'Always',
          'Only when the columns of B are linearly independent',
          'Only when B is square',
          'Only when there are as many x’s as λ’s',
        ],
        answer: 1,
        explain:
          'The proof reaches Bv = 0 and concludes v = 0, and that step is only available because the columns of B are independent. With a dependent B — an overcomplete dictionary, for instance — independent coefficient vectors can produce dependent x vectors.',
      },
      {
        q: 'You have m = 6 vectors, each built from k = 4 independent vectors. What can you conclude?',
        options: [
          'They are independent',
          'They are dependent, from the shape alone',
          'Nothing without elimination',
          'They form a basis of a 6-dimensional space',
        ],
        answer: 1,
        explain:
          'The coefficient matrix is 4 × 6, so there are at most 4 pivots and at least 2 columns without one. Each of those gives a non-trivial combination reaching 0. Note the argument only works one way: m ≤ k proves nothing at all.',
      },
      {
        q: 'Is every linearly independent set a basis of the space it lives in?',
        options: [
          'Yes, independence is the definition',
          'No — it can be independent and still too small to span',
          'Yes, provided none of the vectors is zero',
          'No — a basis must be the canonical one',
        ],
        answer: 1,
        explain:
          'Slide 37 makes exactly this point with three independent vectors in ℝ⁴. A basis needs both halves: independent AND generating. Three vectors reach only a three-dimensional slice of a four-dimensional space.',
      },
      {
        q: 'The space span{(0, 1)ᵀ} ⊆ ℝ². What is its dimension?',
        options: ['2, because the vectors have two components', '1', '0', 'Undefined'],
        answer: 1,
        explain:
          'Slide 39, the whole point of it. Every vector in the space has two components, but the basis has one member, because everything in the space is a multiple of (0, 1)ᵀ. Dimension counts basis vectors, not components — which is the premise of every dimensionality reduction method.',
      },
      {
        q: 'Elimination on the spanning vectors of 𝒰 gives pivots in columns 1, 2 and 4. What is a basis of 𝒰?',
        options: [
          'The first three columns of the echelon form',
          'The original spanning vectors x₁, x₂ and x₄',
          'All four spanning vectors',
          'Columns 1, 2 and 4 of the echelon form',
        ],
        answer: 1,
        explain:
          'Slide 40, step 3, and the trap is real: the basis is made of the ORIGINAL vectors sitting at the pivot positions. The echelon form is only a device for finding out which ones to keep — its own columns are not vectors of 𝒰 in general.',
      },
      {
        q: 'Why does PCA subtract the mean before finding components?',
        options: [
          'To make the numbers smaller',
          'Because a subspace must contain the origin, and uncentred data lies near a flat set that does not',
          'Because the covariance matrix must be square',
          'It is optional and makes no difference',
        ],
        answer: 1,
        explain:
          'PCA looks for a subspace, and a subspace is closed under scaling, so it must pass through 0. Uncentred data lies near an affine set that misses the origin, and the first component then points at the mean instead of along the direction of greatest spread — a failure that still produces plausible-looking output.',
      },
      {
        q: 'A design matrix has three columns and it turns out that x₃ = −x₁ + 2x₂. Would a correlation heat map have shown this?',
        options: [
          'Yes, the correlation between x₁ and x₃ would be 1',
          'Not necessarily — a three-way dependency need not show as any large pairwise correlation',
          'Yes, provided the data is standardised',
          'No, because correlations cannot detect linear relationships',
        ],
        answer: 1,
        explain:
          'This is the chapter’s final example. Collinearity is a statement about the rank of the whole matrix, not about pairs of columns, so pairwise correlations can all look unremarkable while an exact dependency exists. Only a rank computation finds it.',
      },
    ],
    exam: [
      {
        q: 'Define a group and an Abelian group. Determine whether (ℤ, +), (ℕ₀, +) and (ℤ, ·) are groups, justifying each answer.',
        meta: 'Definition plus three tests · 5–6 marks',
        points: [
          'A group is a set G together with an operation ⊗ : G × G → G satisfying four properties.',
          'Closure: ∀ x, y ∈ G, x ⊗ y ∈ G.',
          'Associativity: ∀ x, y, z ∈ G, (x ⊗ y) ⊗ z = x ⊗ (y ⊗ z).',
          'Neutral (identity) element: ∃ e ∈ G such that ∀ x ∈ G, x ⊗ e = x.',
          'Inverse element: ∀ x ∈ G, ∃ y ∈ G with x ⊗ y = y ⊗ x = e.',
          'If in addition x ⊗ y = y ⊗ x for all x, y ∈ G, the group is called Abelian.',
          '(ℤ, +) is an Abelian group: sums of integers are integers, addition is associative and commutative, e = 0, and the inverse of x is −x ∈ ℤ.',
          '(ℕ₀, +) is not a group: closure, associativity and e = 0 hold, but for n > 0 the inverse −n ∉ ℕ₀.',
          '(ℤ, ·) is not a group: closed, associative, identity e = 1, but only 1 and −1 have multiplicative inverses in ℤ.',
          'State clearly that failing a single property is enough to disqualify a candidate.',
        ],
      },
      {
        q: 'Define a real-valued vector space. State the test for a subset to be a vector subspace and explain why the test is shorter than the full definition.',
        meta: 'Definition plus the subspace criterion · 6–8 marks',
        points: [
          'A real-valued vector space is 𝒱 = (𝒱, +, ·) with an inner operation + : 𝒱 × 𝒱 → 𝒱 and an outer operation · : ℝ × 𝒱 → 𝒱.',
          '(𝒱, +) must be an Abelian group; its neutral element is the zero vector 0 = [0, …, 0]ᵀ.',
          'Distributivity: λ·(x + y) = λ·x + λ·y and (λ + ψ)·x = λ·x + ψ·x, for all λ, ψ ∈ ℝ and x, y ∈ 𝒱.',
          'Associativity of the outer operation: λ·(ψ·x) = (λψ)·x for all λ, ψ ∈ ℝ, x ∈ 𝒱.',
          'Neutral element of the outer operation: 1·x = x for all x ∈ 𝒱.',
          'Subspace test: for 𝒰 ⊆ 𝒱 with 𝒰 ≠ Φ, 𝒰 is a subspace if λx ∈ 𝒰 for all λ ∈ ℝ, x ∈ 𝒰, and x + y ∈ 𝒰 for all x, y ∈ 𝒰.',
          'The test is shorter because associativity, distributivity, commutativity and the neutral element are statements that hold for all elements of 𝒱, hence automatically for all elements of 𝒰 ⊆ 𝒱; the operations are inherited, not redefined.',
          'Only closure can fail on passing to a subset, so only closure needs checking.',
          'Note that 𝒰 ≠ Φ together with closure under scaling forces 0 ∈ 𝒰, by taking λ = 0.',
        ],
      },
      {
        q: 'Show that the nullspace of a matrix A, that is {x : Ax = 0}, is a vector subspace. State one consequence for fitting a linear model.',
        meta: 'Three-line proof plus interpretation · 4–5 marks',
        points: [
          'Non-empty: A0 = 0, so 0 is in the set.',
          'Closed under addition: if Ax = 0 and Ay = 0 then A(x + y) = Ax + Ay = 0 + 0 = 0.',
          'Closed under scalar multiplication: if Ax = 0 and λ ∈ ℝ then A(λx) = λ(Ax) = λ·0 = 0.',
          'All three conditions of the subspace test hold, so null(A) is a subspace of ℝⁿ.',
          'The argument uses only the linearity of the map x ↦ Ax, so it holds for every A, of any shape — no invertibility or rank assumption is needed.',
          'Consequence: if the design matrix X has a non-trivial nullspace, then for any weight vector w and any n ∈ null(X), w and w + n give identical predictions on every training sample.',
          'Hence the least-squares fit is not unique, individual coefficients cannot be interpreted, and a further criterion — minimum norm, or an L2 penalty as in ridge regression — is needed to select one solution.',
        ],
      },
      {
        q: 'Define linear dependence and independence. State the three facts of slide 17 and justify the one about the zero vector.',
        meta: 'Definition plus three facts · 5–6 marks',
        points: [
          'Given a vector space 𝒱 and x₁, …, xₖ ∈ 𝒱, the vectors are linearly dependent if there exist λ₁, …, λₖ, not all zero, with Σᵢ λᵢxᵢ = 0.',
          'They are linearly independent if Σᵢ λᵢxᵢ = 0 forces λᵢ = 0 for every i — that is, only the trivial combination reaches the zero vector.',
          'Fact 1: any set of k vectors is either linearly independent or linearly dependent; there is no third possibility.',
          'Fact 2: if at least one of the vectors is the zero vector, the set is linearly dependent.',
          'Justification of fact 2: suppose xⱼ = 0. Choose λⱼ = 1 and λᵢ = 0 for i ≠ j. Then Σᵢ λᵢxᵢ = 1·0 = 0, and not all coefficients are zero, so the definition of dependence is satisfied.',
          'Fact 3: if all the vectors are non-zero, they are linearly dependent if and only if one of them can be written as a linear combination of the others.',
          'Interpretation: dependence means at least one vector is redundant — it adds no reach the others do not already provide.',
        ],
      },
      {
        q: 'Describe how Gaussian elimination is used to test a set of vectors for linear independence, and use it on x₁ = (1, 2, −3, 4)ᵀ, x₂ = (1, 1, 0, 2)ᵀ, x₃ = (−1, −2, 1, 1)ᵀ.',
        meta: 'Method plus a worked example · 6–8 marks',
        points: [
          'Write the vectors as the columns of a matrix; rows correspond to components, columns to vectors.',
          'Reduce to row-echelon form by Gaussian elimination.',
          'The pivot columns identify a linearly independent subset; every non-pivot column is a linear combination of the pivot columns to its left.',
          'If every column is a pivot column, the whole set is linearly independent.',
          'For the given vectors the matrix is the 4 × 3 array with rows (1, 1, −1), (2, 1, −2), (−3, 0, 1), (4, 2, 1).',
          'Eliminating gives the row-echelon form with rows (1, 1, −1), (0, 1, 0), (0, 0, 1), (0, 0, 0).',
          'All three columns are pivot columns, so x₁, x₂, x₃ are linearly independent: the only combination giving the zero vector is λ₁ = λ₂ = λ₃ = 0.',
          'Note that row-echelon form is not unique — scaling a row gives another valid one — but the pivot positions, and hence the rank and the conclusion, are.',
        ],
      },
      {
        q: 'The vectors x₁ … xₘ are built from linearly independent vectors b₁ … bₖ as xⱼ = Σᵢ λᵢⱼbᵢ. Prove that the xⱼ are linearly independent if and only if the coefficient vectors λⱼ are.',
        meta: 'Proof · 6–8 marks',
        points: [
          'Collect the ingredients into B = [b₁, b₂, …, bₖ], so that each equation becomes xⱼ = Bλⱼ.',
          'Test the xⱼ for independence: suppose Σⱼ ψⱼxⱼ = 0.',
          'Substitute: Σⱼ ψⱼxⱼ = Σⱼ ψⱼ(Bλⱼ) = B(Σⱼ ψⱼλⱼ), using linearity of the matrix–vector product.',
          'Write v = Σⱼ ψⱼλⱼ, so the condition becomes Bv = 0.',
          'Bv is a linear combination of the columns of B, which are b₁ … bₖ and are assumed linearly independent; therefore the only combination of them reaching 0 is the trivial one, so v = 0.',
          'Hence Σⱼ ψⱼxⱼ = 0 implies Σⱼ ψⱼλⱼ = 0, and conversely Σⱼ ψⱼλⱼ = 0 gives Σⱼ ψⱼxⱼ = B0 = 0.',
          'So a non-trivial ψ annihilates the xⱼ exactly when the same ψ annihilates the λⱼ: the xⱼ are linearly independent if and only if λ₁ … λₘ are.',
          'State explicitly where the hypothesis is used: the step v = 0 requires the columns of B to be independent, and the result fails without it.',
        ],
      },
      {
        q: 'Explain why m vectors formed from k linearly independent vectors must be linearly dependent when m > k.',
        meta: 'Counting argument · 4–5 marks',
        points: [
          'By the previous result, testing the m vectors reduces to testing the columns of the k × m matrix of coefficients.',
          'Each pivot occupies its own row and its own column, so the number of pivots is at most the number of rows, k.',
          'Since m > k, the number of pivots is at most k < m, so at least m − k columns receive no pivot.',
          'Every non-pivot column can be written as a linear combination of the pivot columns to its left.',
          'Rearranging that relation gives a linear combination of the columns, with a non-zero coefficient on the non-pivot column, equal to the zero vector.',
          'Hence the coefficient vectors are linearly dependent, and therefore so are the m vectors themselves.',
          'The conclusion follows from the shape alone, with no arithmetic on the entries. The converse does not hold: m ≤ k gives no conclusion either way.',
        ],
      },
      {
        q: 'Define generating set, span and basis. State the four equivalent characterisations of a basis, and explain why a linearly independent set need not be a basis.',
        meta: 'Definitions plus characterisations · 6–8 marks',
        points: [
          'For 𝒜 = {x₁, …, xₖ} ⊆ 𝒱, the span of 𝒜 is the set of all linear combinations of its members.',
          '𝒜 is a generating set of 𝒱 if every v ∈ 𝒱 can be written as a linear combination of x₁ … xₖ; we then write 𝒱 = span[𝒜].',
          'A generating set 𝒜 is minimal if no proper subset 𝒜̃ ⊂ 𝒜 still spans 𝒱.',
          'Every linearly independent generating set of 𝒱 is minimal and is called a basis of 𝒱.',
          'Equivalent characterisations: ℬ is a basis; ℬ is a minimal generating set; ℬ is a maximal linearly independent set, so adding any vector makes it dependent; every x ∈ 𝒱 is a linear combination of members of ℬ and that combination is unique.',
          'Uniqueness stated precisely: if x = Σᵢ λᵢbᵢ = Σᵢ ψᵢbᵢ then λᵢ = ψᵢ for all i.',
          'A linearly independent set need not be a basis because it may fail to span: independence is only half the definition.',
          'Example: (1, 2, 3, 4)ᵀ, (2, −1, 0, 2)ᵀ, (1, 1, 0, 4)ᵀ are linearly independent in ℝ⁴ but span only a three-dimensional subspace, so they are not a basis of ℝ⁴.',
          'A basis is also not unique: ℝ³ has the canonical basis, the basis (1,0,0)ᵀ, (1,1,0)ᵀ, (1,1,1)ᵀ, and infinitely many others — but every basis of a given space has the same number of members.',
        ],
      },
      {
        q: 'Define the dimension of a vector space and explain, with an example, why it is not the number of components of its vectors. State the relationship between dim(𝒰) and dim(𝒱) when 𝒰 ⊆ 𝒱.',
        meta: 'Definition plus counterexample · 4–6 marks',
        points: [
          'In a finite-dimensional vector space 𝒱, dim(𝒱) is the number of vectors in a basis of 𝒱.',
          'This is well defined because every basis of a given space has the same number of members.',
          'The number of components of a vector is a property of how it is written, not of the space it belongs to.',
          'Example: 𝒰 = span{(0, 1)ᵀ} ⊆ ℝ². Every vector of 𝒰 has two components, but every one of them is a multiple of (0, 1)ᵀ, so a basis of 𝒰 has one member and dim(𝒰) = 1.',
          'If 𝒰 ⊆ 𝒱 then dim(𝒰) ≤ dim(𝒱).',
          'Moreover dim(𝒰) = dim(𝒱) if and only if 𝒰 = 𝒱, which gives a standard method for proving two spaces are equal: show containment and equal dimension.',
          'This course considers only finite-dimensional vector spaces.',
        ],
      },
      {
        q: 'State the three-step method for finding a basis of 𝒰 = span(x₁, …, xₘ) ⊆ ℝⁿ. Apply it to x₁ = (1, 2, −1, −1, −1)ᵀ, x₂ = (2, −1, 1, 2, −2)ᵀ, x₃ = (3, −4, 3, 5, −3)ᵀ, x₄ = (−1, 8, −5, −6, 1)ᵀ and state dim 𝒰.',
        meta: 'Method plus the deck’s final example · 8–10 marks',
        points: [
          'Step 1: write the spanning vectors as the columns of a matrix A.',
          'Step 2: determine the row-echelon form of A by Gaussian elimination.',
          'Step 3: the spanning vectors associated with the pivot columns form a basis of 𝒰 — the original vectors, not the columns of the echelon form.',
          'Here A is 5 × 4, with rows (1, 2, 3, −1), (2, −1, −4, 8), (−1, 1, 3, −5), (−1, 2, 5, −6), (−1, −2, −3, 1).',
          'Elimination gives rank 3, with pivots in columns 1, 2 and 4; column 3 receives no pivot.',
          'Therefore x₁, x₂, x₃, x₄ are linearly dependent and do NOT constitute a basis of 𝒰.',
          'A basis of 𝒰 is {x₁, x₂, x₄}, and dim 𝒰 = 3.',
          'The reduced row-echelon form has third column (−1, 2, 0)ᵀ above the zero rows, giving the explicit dependency x₃ = −x₁ + 2x₂.',
          'Check by substitution: −x₁ + 2x₂ = (−1, −2, 1, 1, 1)ᵀ + (4, −2, 2, 4, −4)ᵀ = (3, −4, 3, 5, −3)ᵀ = x₃.',
          'Say that {x₁, x₂, x₄} is a basis, not the basis: {x₁, x₃, x₄} also works. What is fixed is the number of members, which is dim 𝒰 = 3.',
        ],
      },
    ],
  },
